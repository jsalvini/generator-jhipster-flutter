...
module.exports = class extends BaseGenerator {
    get initializing() {
        return {
            ...
            displayLogo() {
                ...
                this.log(`${chalk.bold.cyan('   ██    ██ ██      ██   ██ ██               ██ ██   ██ ')}`);
                this.log(`${chalk.bold.cyan('   ██    ██ ██      ██   ██ ███████     ███████ ██   ██ ')}`);
                this.log(`\nWelcome to Tipre-Flutter ${chalk.bold.yellowBright(`v${packagejs.version}`)} !`);
            ...
            checkJhipster() {
                ...
                if (!semver.satisfies(currentJhipsterVersion, minimumJhipsterVersion)) {
                    this.warning(
                        `\nYour generated project used JHipster version (${currentJhipsterVersion})... you need at least (${minimumJhipsterVersion})\n`
                    );
                }
            }
        };
    ...
    install() {
        ...
        if (this.enableTranslation) {
            ...
            this.spawnCommandSync('flutter', ['pub', 'add', 'intl_utils', '^2.8.7'], { cwd: MAIN_DIR });
            this.log(chalk.green('Generate I18n files...'));
            this.spawnCommandSync('flutter', ['pub', 'run', 'intl_utils:generate'], { cwd: MAIN_DIR });
        }
    }
